...
def detect_device() -> Dict[str, Any]:
    ...
    force_dtype = os.environ.get("OLLAMA_FORCE_DTYPE")
    dtype = None
    if force_dtype:
        normalized = force_dtype.strip().lower()
        mapping = {
        ...
        if dtype is None:
            LOGGER.warning("Unknown OLLAMA_FORCE_DTYPE value: %s", force_dtype)
    if dtype is None:
        if device == "cpu":
            dtype = torch.float32
        elif is_rocm or cuda_available:
            dtype = torch.float16
        else:
            dtype = torch.float32
